...
void CACHE::invalidate_lower(uint32_t set,uint32_t way, uint8_t cache_type, uint64_t instr_id, uint32_t cpu_index){
  uint64_t addr = block[set][way].address;
  uint64_t tag = block[set][way].tag;

  if(cache_type==IS_LLC){

    for(int i=0;i<NUM_CPUS;i++){

      CACHE* l2c = next_level_d[i];
      uint32_t l2set = l2c->get_set(addr);
      uint32_t l2way = l2c->get_way(addr,l2set);
      CACHE* l1 = l2c->next_level_d[0];
      uint32_t l1set = l1->get_set(addr);
      uint32_t l1way = l1->get_way(addr,l1set);

      if(l2way!=l2c->NUM_WAY){
        if(l1way!=l1->NUM_WAY){
          if(block[set][way].valid==1 && block[set][way].dirty == 0){   //llc is clean

            if(l2c->block[l2set][l2way].valid == 1 && l2c->block[l2set][l2way].dirty == 0){     //l2 is clean
              if(l1->block[l1set][l1way].dirty == 1){   //l1 is dirty
                l1->wb_dram(l1, l1set, l1way, instr_id, i);
              }
              l1->block[l1set][l1way].valid = 0;    //invalidating everything
              l2c->block[l2set][l2way].valid = 0;
              block[set][way].valid = 0;
            }

            if(l2c->block[l2set][l2way].dirty == 1){    //l2 is dirty
              if(l1->block[l1set][l1way].dirty == 0){   //l1 is not dirty that is clean or invalid
                l2c->wb_dram(l2c, l2set, l2way, instr_id, i);
              }
              else{                                     //l1 is dirty
                l1->wb_dram(l1, l1set, l1way, instr_id, i);
              }
            ...
            if(l2c->block[l2set][l2way].valid == 0){
              if(l1->block[l1set][l1way].valid == 1){
                assert(l1->block[l1set][l1way].valid==0);
              }
              else{
                block[set][way].valid = 0;
              }
          ...
          if(block[set][way].dirty == 1){   //llc is dirty
            if(l2c->block[l2set][l2way].valid == 1 && l2c->block[l2set][l2way].dirty == 0){   //l2 is clean
              if(l1->block[l1set][l1way].dirty == 0){   //l2 is clean
                wb_dram(this, set, way, instr_id, i);
              }
              else{
                l1->wb_dram(l1, l1set, l1way, instr_id, i);
              }
            ...
            if(l2c->block[l2set][l2way].dirty == 1){    //l2 is dirty
              if(l1->block[l1set][l1way].dirty == 0){   //l1 is not dirty
                l2c->wb_dram(l2c, l2set, l2way, instr_id, i);
              }
              else{
                l1->wb_dram(l1, l1set, l1way, instr_id, i);
            ...
            if(l2c->block[l2set][l2way].valid == 0){      //l2 is invalid
              if(l1->block[l1set][l1way].valid == 0){     //l1 is invalid
                wb_dram(this, set, way, instr_id, i);
                block[set][way].valid = 0;
              }
              else{
          ...
          if(block[set][way].valid == 0){
            if(l2c->block[l2set][l2way].valid == 1){
              assert(l2c->block[l2set][l2way].valid == 0);
            }
            else{
              if(l1->block[l1set][l1way].valid == 1){
                assert(l1->block[l1set][l1way].valid == 0);
          ...
          assert(l1way==l1->NUM_WAY);
        }
      }
    }
  }
  else if(cache_type == IS_L2C){
    CACHE* l1 = next_level_d[0];
    uint32_t l1set = l1->get_set(addr);
    uint32_t l1way = l1->get_way(addr, l1set);
    if(l1way!=NUM_WAY){
      if(block[set][way].valid == 1 && block[set][way].dirty == 0){
        if(l1->block[l1set][l1way].dirty == 1){
          l1->wb_prev_level(this, l1set, l1way, instr_id, cpu_index);
        }
        block[set][way].valid = 0;
        l1->block[l1set][l1way].valid = 0;
      }

      else if(block[set][way].dirty == 1){
        if(l1->block[l1set][l1way].dirty == 0){
          wb_prev_level(this, set, way, instr_id, cpu_index);
        }
        else{
          l1->wb_prev_level(this, l1set, l1way, instr_id, cpu_index);
        }
...
void CACHE::wb_dram(CACHE *c, uint32_t set, uint32_t way, uint64_t instr_id, uint32_t cpu_index){
  if(c->to_dram->get_occupancy(2, c->block[set][way].address) == c->to_dram->get_size(2, c->block[set][way].address))
  {
    c->to_dram->increment_WQ_FULL(c->block[set][way].address);
  }
  else{
    PACKET wp;
    ...
    wp.cpu = cpu_index;
    wp.address = c->block[set][way].address;
    wp.full_addr = c->block[set][way].full_addr;
    wp.data = c->block[set][way].data;
    wp.instr_id = instr_id;
    wp.ip = 0;
    wp.type = WRITEBACK;
    wp.event_cycle = current_core_cycle[cpu_index];

    c->to_dram->add_wq(&wp);
  }  
}

void CACHE::wb_prev_level(CACHE *c, uint32_t set, uint32_t way, uint64_t instr_id, uint32_t cpu_index){
  if(c->prev_level->get_occupancy(2, c->block[set][way].address) == c->prev_level->get_size(2, c->block[set][way].address)){
    c->prev_level->increment_WQ_FULL(c->block[set][way].address);
  } 
  else{
    PACKET wp;
    ...
    wp.fill_level = FILL_DRAM;
    wp.cpu = cpu_index;
    wp.address = block[set][way].address;
    wp.full_addr = block[set][way].full_addr;
    wp.data = block[set][way].data;
    wp.instr_id = instr_id;
    wp.ip = 0;
    wp.type = WRITEBACK;
    wp.event_cycle = current_core_cycle[cpu_index];

    c->prev_level->add_wq(&wp);
  } 
}

void CACHE::exclusive_fill(uint32_t set, uint32_t way, uint8_t cache_type, uint64_t instr_id, uint32_t cpu_index){
...
void CACHE::wb_next_level(CACHE *c, uint32_t set, uint32_t way, uint64_t instr_id, uint32_t cpu_index){
  if(c->next_level_d[0]->get_occupancy(2, c->block[set][way].address) == c->next_level_d[0]->get_size(2, c->block[set][way].address)){
    c->next_level_d[0]->increment_WQ_FULL(c->block[set][way].address);
  } 
  else{
    PACKET wp;
    ...
    c->next_level_d[0]->add_wq(&wp);
  }
}

void CACHE::exclusive_read_hit(uint32_t set, uint32_t way, uint8_t cache_type, uint64_t instr_id, uint32_t cpu_index){
  if(cache_type == IS_LLC){
    wb_next_level(this, set, way, instr_id, cpu_index);
    block[set][way].valid = 0;
  }
  if(cache_type == IS_L2C){
    wb_next_level(this, set, way, instr_id, cpu_index);
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
      ...
      if(INC_POLICY == NON_INCLUSIVE){
        ...
        if(INC_POLICY==INCLUSIVE){
          //MODIFICATION
          invalidate_lower(set,way,cache_type,MSHR.entry[mshr_index].instr_id, fill_cpu);
        }
        else if(INC_POLICY==EXCLUSIVE){
          //pass
          exclusive_fill(set, way, cache_type, MSHR.entry[mshr_index].instr_id, fill_cpu);
        }
        else if(INC_POLICY==NON_INCLUSIVE){
          //pass
        }
...
void CACHE::handle_read()
{
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (read_cpu == NUM_CPUS)
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            if (way >= 0) { // read hit
                ...
                // else{
                  // exclusive_read_hit(set, way, cache_type, RQ.entry[index].instr_id, read_cpu);
                // }
      ...
      if(INC_POLICY == EXCLUSIVE){
        block[set][way].valid = 0;    //modified for exclusive
      }  
...
int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata)
{
    ...
    if (PQ.occupancy < PQ.SIZE) {
	    ...
	    pf_packet.pf_metadata = prefetch_metadata;
            ...
            pf_issued++;
            block[get_set(pf_addr)][get_way(pf_addr,get_set(pf_addr))].prefetch = 1;
            return 1;
        }
    }
