...
//printf("Insert  num_adjacent_page: %u  vpage: %lx  ppage: %lxn", num_adjacent_page, vpage, random_ppage);
...
//printf("Found  vpage: %lx  random_ppage: %lxn", vpage, pr->second);
}
...
printf("Off-chip DRAM Size: %u MB Channels: %u Width: %u-bit Data Rate: %u MT/sn",
...
if (found_traces)
{
printf("CPU %d runs %sn", count_traces, argv[i]);
...
//printf ("%s %dn", pch[count_str], count_str);
count_str++;
pch[count_str] = strtok (NULL, " /,.-");
}

//printf("max count_str: %dn", count_str);
//printf("application: %sn", pch[count_str-3]);

int j = 0;
while (pch[count_str-3][j] != '0') {
seed_number += pch[count_str-3][j];
//printf("%c %d %dn", pch[count_str-3][j], j, seed_number);
j++;
}

ooo_cpu[count_traces].trace_file = popen(ooo_cpu[count_traces].gunzip_command, "r");
if (ooo_cpu[count_traces].trace_file == NULL) {
printf("n*** Trace file not found: %s ***nn", argv[i]);
assert(0);
}

count_traces++;
if (count_traces > NUM_CPUS) {
printf("n*** Too many traces for the configured number of cores ***nn");
assert(0);
}
}
...
if (count_traces != NUM_CPUS) {
printf("n*** Not enough traces for the configured number of cores ***nn");
assert(0);
}
...
ooo_cpu[i].ITLB.prev_level = &ooo_cpu[i].STLB;
...
ooo_cpu[i].DTLB.prev_level = &ooo_cpu[i].STLB;
...
ooo_cpu[i].STLB.next_level_i[i] = &ooo_cpu[i].ITLB;
ooo_cpu[i].STLB.next_level_d[i] = &ooo_cpu[i].DTLB;
...
ooo_cpu[i].L1I.prev_level = &ooo_cpu[i].L2C;
...
ooo_cpu[i].L1D.prev_level = &ooo_cpu[i].L2C;
ooo_cpu[i].L1D.to_dram = &uncore.DRAM;
...
ooo_cpu[i].L2C.next_level_i[i] = &ooo_cpu[i].L1I;
ooo_cpu[i].L2C.upper_level_dcache[i] = &ooo_cpu[i].L1D;
ooo_cpu[i].L2C.next_level_d[0] = &ooo_cpu[i].L1D;
ooo_cpu[i].L2C.lower_level = &uncore.LLC;
ooo_cpu[i].L2C.prev_level = &uncore.LLC;
ooo_cpu[i].L2C.to_dram = &uncore.DRAM;
...
uncore.LLC.next_level_i[i] = &ooo_cpu[i].L2C;
uncore.LLC.upper_level_dcache[i] = &ooo_cpu[i].L2C;
uncore.LLC.next_level_d[i] = &ooo_cpu[i].L2C;
uncore.LLC.lower_level = &uncore.DRAM;
uncore.LLC.to_dram = &uncore.DRAM;
// uncore.LLC.prev_level = &uncore.DRAM;
